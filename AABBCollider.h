...
{
public:
	AABBCollider(GameObject* owner, const Vector2& size);
	AABBCollider(GameObject* owner, const Vector2& leftTop, const Vector2& rightBottom);
	AABBCollider(GameObject* owner, float width, float height); //직사각형
	AABBCollider(GameObject* owner, float left, float top, float right, float bottom);
